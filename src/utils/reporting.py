...
# Note: FunctionalityNode might be needed if we pass objects instead of dicts later,
# but for now, assuming dicts based on current main.py usage.
# from chatbot_explorer.functionality_node import FunctionalityNode


def print_structured_functionalities(
    f, nodes: List[Dict[str, Any]], indent: str = ""
):
...
def generate_graph_image(
    ...
    dot.node(
        start_node_name,
        label="", shape="circle", width="0.3", height="0.3",
...
def write_yaml_profiles(output_dir: str, profiles: List[Dict[str, Any]]):
    ...
    for profile in profiles:
        ...
        if not profile_name or not isinstance(profile_name, str):
            print(f"   WARN: Skipping profile - missing or invalid 'profile_name'. Data: {profile}")
            error_count += 1
            continue
        if not profile_content or not isinstance(profile_content, dict):
             print(f"   WARN: Skipping profile '{profile_name}' - missing or invalid 'profile_content'. Expected dict, got {type(profile_content)}")
             error_count += 1
             continue

        # Sanitize filename (replace spaces, ensure valid characters)
        safe_filename = "".join(c if c.isalnum() or c in ('_', '-') else '_' for c in profile_name.replace(' ', '_'))
        filename = f"{safe_filename}.yaml"
        filepath = os.path.join(output_dir, filename)

        try:
            with open(filepath, "w", encoding="utf-8") as f:
                yaml.dump(profile_content, f, allow_unicode=True, sort_keys=False, default_flow_style=False, width=1000)
            ...
            print(f"   ERROR: An unexpected error occurred while writing profile '{profile_name}': {e}")
            error_count += 1

    print(f"   Finished writing profiles: {profile_count} successful, {error_count} errors.")
