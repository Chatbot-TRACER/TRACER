        ...
        **RULE: Your DEFAULT action MUST be to create separate root nodes (empty `parent_names`: `[]`). Only create parent-child links if the conversational evidence for dependency is EXPLICIT, CONSISTENT, and UNDENIABLE.** Avoid forcing hierarchies onto informational interactions.
        ...
        try:
            ...
            if potentially_misplaced:
                ...
                # Ask LLM to review these specific nodes
                correction_prompt = f"""
                ...
                {json.dumps(root_nodes_dicts, indent=2)[:1000]} # Show current structure context
                ...
                [
                    {{
                        ...
                        "should_be_child_of": ["parent1", "parent2"] # List of parent names or empty list
                    }},
                    ...
                ]
                """
                ...
                try:
                    ...
                    if match:
                        ...
                        for correction in corrections:
                            ...
                            if (
                                node_name in nodes_map and parents
                            ):  # If parents are suggested
                                print(
                                    f"   Correcting: '{node_name}' should be child of {parents}"
                                )

                                # Update the node's parent list
                                nodes_map[node_name]["parent_names"] = parents

                                # Re-link parent-child relationships in the map
                                for parent_name in parents:
                                    if parent_name in nodes_map:
                                        # Remove node from roots if it was there
                                        root_nodes_dicts = [
                                            n
                                        ...
                                        # Add as child to the suggested parent(s)
                        ...
                        # Recalculate root nodes after applying corrections
                        all_child_names = set()
                        for node_info in nodes_map.values():
                            for child_info in node_info.get("children", []):
                                all_child_names.add(child_info["name"])
